...
### 3.5 非物理アーティファクト → 対策実装済み(2026-07-12, `aa745dd`)
当時は feasibility 判定が `CD>1e-6` のみのため、超薄翼で SA が CD≈0 を返し Cl/Cd=4707
（CMA-ES）, 1142（Random）等の非物理値が混入していた。現在はメッシュ生成前の
最小厚み・最小面積・自己交差の fail-fast 検証と、`min_cd=1e-3` による事後的な
非物理値排除の両方を実装済み(BENCHMARK.md §20.3-20.4)。3D翼(front-wing, §24)の
粗いメッシュでは残存リスクがゼロではない点に留意。
