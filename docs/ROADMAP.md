...
### ~~MF-2~~: SU2 実ペアでのカスケード検証 → **相関ゲート不通過で中止(2026-07-12)**
- 実測(`BENCHMARK.md` §19): NeuralFoil xlarge ↔ SU2 RANS は R²=0.284・ρ=0.689・
  feasibility一致66-69% — 文献成立条件(R²>0.75)を大きく下回り、カスケードA/Bを中止
  (約17時間のSU2計算を回避)。**NeuralFoil→SU2カスケードは現構成では非推奨**。

### MF-2': 代替LFペアの探索(MF-2の後継)
- 候補: (a) **LF=SU2粗メッシュ**(同物理・低解像度 — 相関が構造的に高いはず。
  su2_runner の n_half/nj/max_iter を落とした設定で §19 と同じ相関ゲートを先に回す)
  (b) NeuralFoilの線形校正(SU2数十点でLF値を回帰補正してから相関再測定)
- いずれも「相関ゲート→合格時のみA/B」の二段構え(§19の教訓)を踏襲。
...
- **§19 の結果で優先度上昇**: 特徴注入型は箱を縮めないため、カスケードと違い
  低相関ペア(ρ≈0.69)でも「弱い事前情報」として安全に効く可能性がある。
  ただし R²=0.284 では利得上限も小さい見込み — MF-2' の相関改善が先。
