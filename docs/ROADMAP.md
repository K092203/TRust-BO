...
## 未着手バックログ(2026-07-13 更新)

実測結果の背景は `BENCHMARK.md` §14–21。完了・棄却済み:
...
~~P2D~~(判定確定・下記)、~~min_cd~~(1e-3実装済み、§20.4)。
...
### ~~CAL~~/BILOG: アンサンブルσ校正・bilog 出力変換
- BILOG は §20.2 で棄却確定(GM 0.881、rosenbrockで0.634)。
- **CAL(σ校正)は未着手のまま次回セッションへ持ち越し**: surrogate.rs 変更を伴う
  中規模作業。ts の質に直結するがσ縮小は探索を殺すリスクがあり慎重な A/B が必要
  (棄却済みの「コヒーレント単一メンバーTS」がσ縮小系の失敗例)。
