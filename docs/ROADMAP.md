...
## 未着手バックログ(2026-07-16 更新)

実測結果の背景は `BENCHMARK.md` §14–24。完了・棄却済み:
...
~~BILOG~~(rosenbrockでGM 0.634・37%悪化、棄却 §20.2)、
~~P2D~~(判定確定・下記)、~~min_cd~~(1e-3実装済み、§20.4)、~~CAL~~(GM 1.008棄却、§22)、
~~実CFDニッチ尖鋭化(候補6/1/9/11)~~(全棄却、§23)、
~~3Dフロントウィング実用検証~~(65D INC_RANS パイプライン成功、§24)。

### ~~MF-2~~ / ~~MF-2'~~: SU2 カスケード検証 → **両LF候補とも相関ゲート不通過、再開見送り(2026-07-13)**
- `BENCHMARK.md` §19: LF=NeuralFoil xlarge → R²=0.284・ρ=0.689(不通過)
- `BENCHMARK.md` §21: LF=SU2粗メッシュ(同物理・低解像度)→ **R²=0.447・ρ=0.423
  (SMOKEでは0.859と出たが本走48点で逆転・不通過)**。教訓: 相関ゲートはSMOKE規模
  (n=8)では確定させず、本走規模(n≥30程度)まで判定を待つこと。
- **確定**: NeuralFoil→SU2系のカスケードは現在の2つのLF定義では成立しない。
  §18.1(70%削減)は同系列サロゲート内(NeuralFoil xsmall↔xxxlarge)限定の成果として確定。

### MF-2'': 忠実度軸を1本に絞ったLF定義(次善策、未着手)
- §21.1 参照。候補: (a) メッシュはHFと同一・反復数のみ削減するLF(交絡除去だが
  コスト比が悪化する可能性) (b) 境界層解像度は保持し遠方場のみ粗くするメッシュ
  (剥離・遷移予測を壊さずセル数だけ削減)
- 着手前に直接20点規模で相関を仮測定し、SMOKE規模の見かけ相関で判断しないこと。
...
- **優先度低下(2026-07-13)**: §19/§21 とも相関が弱く(R²=0.28–0.45)、
  「弱い事前情報」としての利得上限も小さい見込み。MF-2'' の相関改善結果を待つ。

### ~~CAL~~/~~BILOG~~: アンサンブルσ校正・bilog 出力変換 → **両方とも棄却確定(2026-07-13)**
- BILOG は §20.2 で棄却確定(GM 0.881、rosenbrockで0.634)。
- **CAL(σ校正)も実装・実測済みで棄却確定**: NLL閉形式解の温度スケーリング
  (縮小方向禁止、[1.0,5.0]クランプ)を実装し4関数×{50,100}D×ノイズ{0,5%}×8seed,
  budget250で検証。GM比 base/cal = 1.008(誤差範囲、§22)。フラグはデフォルトoffで
  残置(棄却済みフラグ、`sigma_calibration`)。

### P2D: phase2_early_frac の実 CFD 検証と v0.3 デフォルト化判断 → **判定確定(2026-07-13)**
- `BENCHMARK.md` §20.1(NeuralFoil 16ペア)・§20.3(SU2実RANS 3シード)とも、
  acquisition="ei" では EI 停滞シグナルが early_frac より先に Phase 2 を発火させるため
  **early_frac は完全に冗長**(全ペアで軌道完全一致)。ts 系には §16.2 の利得(GM 1.372)。
- **推奨確定: v0.3.0 で `enable_phase2=True` 時のデフォルトを `phase2_early_frac=0.25` に**。
  ei 利用者に副作用ゼロ・ts 利用者に利得のみ。ビット挙動が変わるのは
  ts+enable_phase2 の組み合わせのみ(minor バージョン境界での変更として妥当)。
