...
> ⚠ **关于 SU2(H-2)示例的说明:** 目前的可行性检查较为简单
> (`Cd > 0` + 网格有效性)。极薄的形状可能蒙混过关，产生非物理的 Cl/Cd 值
> (即上表中标 ⚠ 的数值)。计划加入几何约束(最小厚度/面积)。
...
- **CFD 可行性检查存在已知缺口:** SU2(H-2)流程现在会在网格生成前对无效几何形状(最小厚度/面积、自相交)进行快速失败检查，并事后拒绝低于阈值的非物理 `Cd` 值，从而封堵了此前观察到的主要一类伪影问题(见 [docs/BENCHMARK.md](docs/BENCHMARK.md) §20.3)。在更粗糙的 3D 前翼网格上(见下文)仍存在一定的残余风险。
